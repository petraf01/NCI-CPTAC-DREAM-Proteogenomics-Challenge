...
correlation_by_row_FINAL_ROUND_sc2 <- function(pred_path, truth_path, protein_path) {
  prediction <- read.csv( pred_path, row.names = 1 , check.names = F, sep="\t") 
  test_prot  <- read.csv( truth_path, row.names = 1 , check.names = F, sep="\t")
  
  common_protein <- rownames(read.csv(protein_path, row.names = 1 , check.names = F, sep="\t"))
  prediction <- prediction[common_protein, colnames(test_prot)]
  test_prot <- test_prot[common_protein, colnames(test_prot)]
...
NRMSE_by_row_FINAL_ROUND_sc2 <- function(pred_path, truth_path, protein_path)  {
  suppressPackageStartupMessages(library(hydroGOF))
  prediction <- read.csv( pred_path, row.names = 1 , check.names = F, sep="\t") 
  test_prot  <- read.csv( truth_path, row.names = 1 , check.names = F, sep="\t")
...
correlation_by_row_FINAL_ROUND_sc3 <- function(pred_path, truth_path, phosphosites_path) {
  prediction <- read.csv( pred_path, row.names = 1 , check.names = F, sep="\t") 
  test_prot  <- read.csv( truth_path, row.names = 1 , check.names = F, sep="\t")
...
NRMSE_by_row_FINAL_ROUND_sc3 <- function(pred_path, truth_path, phosphosites_path)  {
  suppressPackageStartupMessages(library(hydroGOF))
  prediction <- read.csv( pred_path, row.names = 1 , check.names = F, sep="\t") 
  test_prot  <- read.csv( truth_path, row.names = 1 , check.names = F, sep="\t")
